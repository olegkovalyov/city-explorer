<!-- resources/js/Components/FavoritesList.vue -->
<script setup>
...
defineProps({
    ...
    isFavorite: { // Function to check favorite status (still needed for the star icon)
        type: Function,
        required: true,
    },
});
...
// Placeholder image SVG data URL (same as in SearchResults)
...
<template>
    <Card>
        ...
        <CardContent>
            ...
            <div v-else-if="favoritePlaces.length > 0" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
                <div v-for="place in favoritePlaces" :key="place.fsq_id || place.id" class="relative">
                    <Card
                    ...
                    >
                        <CardHeader class="p-0">
                            <img
                                v-if="place.photo_url || (place.photos && place.photos.length > 0)"
                                :src="place.photo_url || place.photos[0]"
                                :alt="place.name"
